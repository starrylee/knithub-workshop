...
public class AuthService {
    ...
    /**
     * 按会话 id 恢复当前登录用户（FT-01-US-03 {@code /auth/me}）。
     *
     * @param sid Cookie {@code sid}
     * @return 有效会话对应用户；sid 缺失/无效或对应用户不存在时为 empty
     */
    ...
    /**
     * 登出销毁会话（FT-01-US-04 {@code /auth/logout}）。幂等：sid 为空或
     * 无效时不做任何事，不抛错。
     */
